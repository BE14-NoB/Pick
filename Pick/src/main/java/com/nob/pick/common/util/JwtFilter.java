...
public class JwtFilter extends OncePerRequestFilter {
    ...
    protected boolean shouldNotFilter(HttpServletRequest request) throws ServletException {
        ...
        boolean shouldNotFilter = path.startsWith("/oauth2/") || path.startsWith("/login/oauth2/") || path.equals("/api/github/callback") || (method.equals("POST") && path.equals("/api/members/email")) || (method.equals("POST") && path.equals("/api/members/password")) || (method.equals("POST") && path.equals("/api/members/check-email")) || (method.equals("POST") && path.equals("/api/members/check-phone")) || (method.equals("POST") && path.equals("/api/members/signup")) || (method.equals("POST") && path.equals("/api/members/login"));
        ...
        if (token != null) {
            try {
                ...
                if (jwtUtil.validateToken(token) && !jwtUtil.isTokenExpired(token) && !tokenBlacklist.isBlacklisted(token)) {
                    ...
                    // ROLE_ 접두사 중복 추가 방지
                    List<String> authorities = roles.stream().map(role -> role.startsWith("ROLE_") ? role : "ROLE_" + role).collect(Collectors.toList());
